...
PROJECT_DIR="/workspaces/adaptive-experimentation"
cd "$PROJECT_DIR"
...
# --- If this is a package repo (src/ exists), do editable install for imports ---
if [ -f "pyproject.toml" ]; then
...
if python -c "import ipykernel" >/dev/null 2>&1; then
  python -m ipykernel install --user --name="adaptive-experimentation" --display-name="Python (adaptive-experimentation)"
fi
